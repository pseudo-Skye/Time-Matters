...
$\\forall C \\in T$, non-self match $M\_D$ of $D$, and non-self match $M\_C$ of $C$, $\\min (\\operatorname{Dist}(D, M\_D))>\\min (\\operatorname{Dist}(C, M\_C))$
